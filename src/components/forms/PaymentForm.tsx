...
const PaymentForm: React.FC<PaymentFormProps> = ({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!stripe || !elements || !selectedRequest) {
      toast({
        title: "Error",
        description: "Stripe is not ready or missing ticket data.",
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/stripe/create-payment-intent`,
        {
          ...
          amount: Math.round(parseFloat(selectedRequest.mco || "0") * 100),
      ...
      toast({
        ...
        description: `Payment of $${selectedRequest.ticketCost} completed.`,
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          {isSubmitting ? 'Processing...' : `Pay $${selectedRequest?.ticketCost}`}
        </button>
      </form>
    </div>
  );
